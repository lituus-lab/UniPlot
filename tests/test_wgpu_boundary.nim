...
suite "WGPU boundary":
  ...
  test "a configured native runtime submits an offscreen render pass":
    ...
    if libraryPath.len == 0:
      ...
      # The path came from `wgpuDeps`, so it exists and loads. What can still
      # go wrong here is the machine: a headless runner offers no adapter, and
      # every check below needs a device to mean anything. Both shapes that
      # can take -- an unavailable backend or a raise from device creation --
      # are the environment speaking, not the boundary under test.
      var backend: WgpuBackend
      var haveDevice = true
      try:
        backend = openWgpuBackend(libraryPath, preparedCacheCapacity = 2)
        haveDevice = backend.state != wbsUnavailable
      except WgpuError as error:
        haveDevice = false
        echo "no WGPU device here: ", error.msg
      if not haveDevice:
        echo "skipped: this machine offers no WGPU adapter"
        skip()
      else:
        ...
        let styledPrepared = styledSpec.compileScene(
            styledSize).prepareWgpuScene(
          font)
